...
func (r *fakeRepository) Save(u *user.User) error {
	r.users[u.ID()] = u
	return nil
}
...
func (r *fakeRepository) FindByEmail(email user.Email) (*user.User, error) {
	for _, u := range r.users {
		if u.Email() == email {
			return u, nil
		}
...
func TestService_CreateUser(t *testing.T) {
	svc := appuser.NewService(newFakeRepository())
	...
	if u.Email().String() != "antonio@antonioeprado.dev" {
		t.Errorf("expected email %q, got %q", "antonio@antonioeprado.dev", u.Email().String())
	}
}

func TestService_CreateUser_MissingName(t *testing.T) {
	svc := appuser.NewService(newFakeRepository())
...
func TestService_UpdateUser_DuplicateEmail(t *testing.T) {
	svc := appuser.NewService(newFakeRepository())
