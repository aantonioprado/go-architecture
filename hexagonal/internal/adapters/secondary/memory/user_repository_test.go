...
func TestInMemoryUserRepository_Create(t *testing.T) {
	...
	user := &domain.User{ID: "1", Name: "Antônio Prado", Email: "antonio@antonioeprado.dev", CreatedAt: time.Now()}
...
func TestInMemoryUserRepository_FindByEmail_NotFound(t *testing.T) {
	...
	if _, err := repo.FindByEmail("missing@antonioeprado.dev"); !errors.Is(err, domain.ErrUserNotFound) {
...
func TestInMemoryUserRepository_FindAll(t *testing.T) {
	...
	first := &domain.User{ID: "1", Name: "Antônio Prado", Email: "antonio@antonioeprado.dev", CreatedAt: time.Now()}
	second := &domain.User{ID: "2", Name: "Another User", Email: "another@antonioeprado.dev", CreatedAt: time.Now()}
...
func TestInMemoryUserRepository_FindById_NotFound(t *testing.T) {
	...
	if _, err := repo.FindById("unknown-id"); !errors.Is(err, domain.ErrUserNotFound) {
		t.Fatalf("expected ErrUserNotFound, got %v", err)
	}
}
...
func TestInMemoryUserRepository_Update(t *testing.T) {
	...
	updated := &domain.User{ID: user.ID, Name: "Antônio Elias Prado", Email: user.Email, CreatedAt: user.CreatedAt}
...
func TestInMemoryUserRepository_Update_NotFound(t *testing.T) {
	...
	user := &domain.User{ID: "unknown-id", Name: "Antônio Prado", Email: "antonio@antonioeprado.dev", CreatedAt: time.Now()}

	if err := repo.Update(user); !errors.Is(err, domain.ErrUserNotFound) {
		t.Fatalf("expected ErrUserNotFound, got %v", err)
	}
}
...
func TestInMemoryUserRepository_Delete(t *testing.T) {
	...
	if _, err := repo.FindById(user.ID); !errors.Is(err, domain.ErrUserNotFound) {
		t.Fatalf("expected ErrUserNotFound, got %v", err)
	}
}
...
func TestInMemoryUserRepository_Delete_NotFound(t *testing.T) {
	...
	if err := repo.Delete("unknown-id"); !errors.Is(err, domain.ErrUserNotFound) {
		t.Fatalf("expected ErrUserNotFound, got %v", err)
	}
}
