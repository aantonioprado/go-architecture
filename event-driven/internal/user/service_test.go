...
func (p *fakePublisher) Publish(event events.Event) {
	p.published = append(p.published, event)
}

func TestUserService_CreateUser(t *testing.T) {
	svc := user.NewUserService(newFakeRepository(), &fakePublisher{})
...
func TestUserService_CreateUser_PublishesUserCreated(t *testing.T) {
	pub := &fakePublisher{}
	svc := user.NewUserService(newFakeRepository(), pub)
	...
	event, ok := pub.published[0].(user.UserCreated)
	if !ok {
		t.Fatalf("expected user.UserCreated, got %T", pub.published[0])
	}

	if event.UserID != created.ID {
		t.Errorf("expected UserID %q, got %q", created.ID, event.UserID)
	}

	if event.Email != "antonio@antonioeprado.dev" {
		t.Errorf("expected email %q, got %q", "antonio@antonioeprado.dev", event.Email)
	}
}
